...
namespace rx_dbc_mysql
{
    ...
    {
        friend class stmt_t;
        uint32_t                 m_max_bulk_deep;               //最大的批量数
        conn_t		        *m_conn;		                //关联的数据库连接对象
        uint16_t                 m_bulk_idx;                     //当前操作的行块序号
        ...
        sql_param_t& set_string(PStr text)
        {
            ...
            {//当前实际数据类型是文本串,赋值的也是文本,那么就进行拷贝赋值吧
                uint16_t data_len = uint16_t(strlen(text));           //得到数据实际长度
                uint8_t *data_buff = m_col_databuff.ptr(m_bulk_idx*m_max_data_size);   //得到可用缓冲区
