...
namespace pgsql
{
    ...
    {
    public:
        env_t() { pgsql_library_init(0, NULL, NULL); }
        ~env_t() { pgsql_library_end(); }
    };

    //-------------------------------------------------
    //获取db更详细的错误信息
    inline bool get_last_error(int32_t &ec, char *buff, uint32_t max_size,pgsql *handle)
    {
        ...
        if (handle)
        {
            ec = pgsql_errno(handle);
            desc << "(DB_ERROR)<" << rx::n2s_t((uint32_t)ec) << ':';
            desc << pgsql_error(handle) << '>';
        }
        else
        {
            ec = 0;
    ...
    //获取DB/STMT更详细的错误信息
    inline bool get_last_error(int32_t &ec, char *buff, uint32_t max_size, pgsql_STMT *handle)
    {
        bool get_details = false;
        rx::tiny_string_t<> desc(max_size, buff);
        ...
        if (handle)
        {
            ec = pgsql_stmt_errno(handle);
            desc << "(DB_ERROR)<" << rx::n2s_t((uint32_t)ec) << ':';
            desc << pgsql_stmt_error(handle) << '>';
        }
        else
        {
            ec = 0;
    ...
    public:
        ...
        error_info_t(pgsql *handle, const char *source_name = NULL, uint32_t line_number = -1, const char *format = NULL, ...)
        {
            int32_t db_ec;
            char msg[1024];
        ...
        //-------------------------------------------------
        //构造函数,通过环境句柄得到db的详细错误信息
        error_info_t(pgsql_STMT *handle, const char *source_name = NULL, uint32_t line_number = -1, const char *format = NULL, ...)
        {
            int32_t db_ec;
            char msg[1024];
            get_last_error(db_ec, msg, sizeof(msg), handle);
        ...
        //-------------------------------------------------
        //判断错误是否为用户名口令错误
        bool is_bad_user_pwd() { return m_pgsql_ec == ER_ACCESS_DENIED_ERROR; }
        //判断是否为连接错误
        bool is_conn_timeout() { return m_pgsql_ec == CR_SERVER_LOST; }
        //连接断开
        bool is_connection_lost() { return m_pgsql_ec == CR_SERVER_LOST; }
        //连接失败(12541监听器不存在;或口令错误;或连接超时;或连接丢失)
        bool is_connect_fail() { return m_pgsql_ec == CR_CONN_HOST_ERROR || m_pgsql_ec == CR_UNKNOWN_HOST || m_pgsql_ec == ER_BAD_DB_ERROR || is_bad_user_pwd() || is_conn_timeout() || is_connection_lost(); }
        //-------------------------------------------------
        //是否为唯一性约束冲突
        bool is_unique_constraint() { return m_pgsql_ec == ER_DUP_ENTRY; }
    };
    ...
    public:
        ...
        datetime_t(const pgsql_TIME& o) { m_Date = o; }
        //-------------------------------------------------
        //设置各个分量
        ...
        //-------------------------------------------------
        datetime_t& operator = (const pgsql_TIME& o) { m_Date = o; return (*this); }
        void to(pgsql_TIME& o) const { o = m_Date; }
        //-------------------------------------------------
